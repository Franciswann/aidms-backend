...
提供一套不依賴特定儲存方式、可分級過濾、非同步寫入、可後續擴展的日誌系統，並整合進 Task 1 的 API 服務作為 HTTP request logging middleware。
...
呼叫者（例如 `LoggingMiddleware`）只認識 `LogManager` 提供的方法（`WriteLog`/`ReadLogs`/`ClearLogs`/`RegisterLogHandler`），不直接接觸任何具體的 Writer/Reader/Handler 實作——這跟 Task 1 一路使用的依賴反轉（DIP）是同一套原則。

### 為什麼 `LogEntry` 是介面而不是 struct

PDF 明確要求 `LogEntry` 是介面。好處：`LogWriter`/`LogReader`/`LogHandler` 三者都只依賴這個介面的方法，不依賴某個具體的內部欄位佈局，未來如果想換一種 entry 的內部表示方式（例如池化重用的版本，見下方「Zero-Allocation」章節），三個介面的實作完全不用改。
...
## [進階] Zero-Allocation / Low GC Pressure（設計方向，未實作）
...
這展示了 Task 2 不是一個孤立的練習題，而是真的能力裝進 Task 1 的服務裡，取代/補強 Gin 預設的 access log。
